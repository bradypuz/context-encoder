...
parser.add_argument('--outf',type=str, default='result',help='path to the training results')

opt = parser.parse_args()
print(opt)

try:
    os.makedirs("%s/train/cropped"%(opt.outf))
    os.makedirs("%s/train/real"%(opt.outf))
    os.makedirs("%s/train/recon"%(opt.outf))
...
half_imageSize = int(opt.imageSize/2)
quarter_imageSize = int(opt.imageSize/4)
...
input_real = torch.FloatTensor(opt.batchSize, opt.nc, imageSize, imageSize)
input_cropped = torch.FloatTensor(opt.batchSize, opt.nc, imageSize, imageSize)
label = torch.FloatTensor(opt.batchSize)
real_label = 1
fake_label = 0

real_center = torch.FloatTensor(opt.batchSize, opt.nc, half_imageSize, half_imageSize)

#generate the alpha matrix and mask
alpha = torch.zeros(opt.batchSize, opt.nc, imageSize, imageSize)
alpha[:,:,quarter_imageSize + opt.overlapPred:quarter_imageSize+half_imageSize - opt.overlapPred,
quarter_imageSize + opt.overlapPred:quarter_imageSize+half_imageSize - opt.overlapPred] = 1.0 #only the center region is 1.0
mean_mat = torch.zeros(opt.batchSize, opt.nc, imageSize, imageSize)
if opt.nc == 3:
    mean_mat[:,0,:,:] = 2*117.0/255.0 - 1.0
    mean_mat[:,1,:,:] = 2*104.0/255.0 - 1.0
    mean_mat[:,2,:,:] = 2*123.0/255.0 - 1.0
...
for epoch in range(resume_epoch,opt.niter):
    for i, data in enumerate(dataloader, 0):
        real_cpu, _ = data
        real_center_cpu = real_cpu[:,:,quarter_imageSize:quarter_imageSize+half_imageSize,quarter_imageSize:quarter_imageSize+half_imageSize]
        batch_size = real_cpu.size(0)
        input_real.data.resize_(real_cpu.size()).copy_(real_cpu)
        input_cropped.data.resize_(real_cpu.size())
        real_center.data.resize_(real_center_cpu.size()).copy_(real_center_cpu)
        alpha.data.resize_(real_cpu.size())
        mean_mat.data.resize_(real_cpu.size())
        ...
        input_cropped = input_real * (1-alpha) + mean_mat*alpha
        ...
        if i % 100 == 0:
            ...
            vutils.save_image(input_cropped.data,
                    '%s/train/cropped/cropped_samples_epoch_%03d.png' % (opt.outf, epoch),  normalize=True)
            ...
            vutils.save_image(recon_image.data,
                    '%s/train/recon/recon_center_samples_epoch_%03d.png' % (opt.outf, epoch),  normalize=True)


    # do checkpointing
    torch.save({'epoch':epoch+1,
                'state_dict':netG.state_dict()},
                '%s/model/netG_streetview.pth'%(opt.outf) )
    torch.save({'epoch':epoch+1,
                'state_dict':netD.state_dict()},
                '%s/model/netlocalD.pth'%(opt.outf) )
